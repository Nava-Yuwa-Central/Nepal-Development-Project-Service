...
async def migrate(context: MigrationContext) -> None:
    ...
    # Process ADB projects next
    await _migrate_adb_projects(context, author_id)

    # Process JICA projects next
    await _migrate_jica_projects(context, author_id)

    # Process EU projects next
    await _migrate_eu_projects(context, author_id)

    # Process MoF DFMIS projects next
    await _migrate_dfmis_projects(context, author_id)
...
async def _migrate_world_bank_projects(context: MigrationContext, author_id: str) -> None:
    ...
    try:
        for project_data in projects:
            ...
            identifiers = []
            if project_id:
                # Only add URL if it's valid and not empty
                url_value = project_data.get("url", "")
                if not url_value and project_id:
                    url_value = f"https://projects.worldbank.org/en/projects/{project_id}"

                # Only create ExternalIdentifier with URL if URL is not empty
                if url_value.strip():
                    identifiers.append(
                        ExternalIdentifier(
                            scheme="other",
                    ...
                    # Create identifier without URL if no valid URL available
                    identifiers.append(
                        ExternalIdentifier(
                            scheme="other",
                            value=str(project_id),
            ...
            if implementing_agency:
                try:
                    ...
                    # Search for existing entity by slug using the search service
                    search_results = await context.search.search_entities(
                        entity_type="organization",
                        query=agency_slug,
                        limit=1
                    )
                    agency_entity = search_results[0] if search_results else None
                    ...
                    if not agency_entity:
                        ...
                        agency_entity = await context.publication.create_entity(
                            entity_type=EntityType.ORGANIZATION,
                            entity_subtype=EntitySubType.GOVERNMENT_ENTITY,
                            entity_data={
                                "slug": agency_slug,
                                "names": [
                                    Name(
    ...
    try:
        for project_data in projects:
            ...
            if project_id:
                ...
                url_value = project_data.get("url", "")
                if not url_value and project_id:
                    url_value = f"https://npbmis.npc.gov.np/projects/{project_id}"
            ...
            try:
                ...
                try:
                    ...
                    if "already exists" in str(e):
                        ...
                        govn_entity = search_results[0] if search_results else None
...
async def _migrate_adb_projects(context: MigrationContext, author_id: str) -> None:
    """
    Import ADB (Asian Development Bank) projects for Nepal from scraped JSON data

    Data source: ADB IATI XML feed converted to standardized JSON format
    """
    context.log("Starting: Importing ADB projects for Nepal")

    # Load projects from pre-scraped data file
    context.log("Loading ADB projects from source data...")
    try:
        projects = context.read_json("source/adb_projects.json")
        context.log(
            f"Loaded {len(projects)} projects from source/adb_projects.json"
        )
    except FileNotFoundError:
        context.log("WARNING: source/adb_projects.json not found.")
        context.log(
            "Please run: python -m migrations.007-source-projects.asian_development_bank.scrape_adb"
        )
        context.log("to scrape and save ADB project data first.")
        context.log("Skipping ADB project import...")
        return

    if not projects:
        context.log(
            "WARNING: No ADB projects in source data. Skipping import."
        )
        return
    ...
    import_date = datetime.now(timezone.utc).date()
    attribution_details = f"Imported from ADB (www.adb.org) on {import_date}"
    attributions = [
        Attribution(
            title=LangText(
                en=LangTextValue(
                    value="ADB Projects", provenance="human"
                ),
                ne=LangTextValue(value="एडीबी परियोजना", provenance="human"),
            ),
            details=LangText(
                en=LangTextValue(value=attribution_details, provenance="human"),
                ne=LangTextValue(
                    value=f"एशियाली विकास बैंक (www.adb.org) बाट {import_date} मा आयात गरिएको",
                    provenance="human",
                ),
            ),
        )
    ...
    # Get all locations for lookup (we can reuse the same lookup as World Bank and NPC)
    locations = await context.search.search_entities(
        entity_type="location", limit=10_000
    )
    location_lookup: Dict[str, object] = {}
    ...
    try:
        for project_data in projects:
            # Extract basic information from ADB data format
            title = (project_data.get("title") or "").strip()
            if not title:
                skipped_count += 1
                continue
            ...
            # Extract funding information
            funding_source = project_data.get("funding_source", "Asian Development Bank")
            total_budget = project_data.get("total_allocated_budget", "")
            spending = project_data.get("real_time_spending", "")
            loan_amount = project_data.get("loan_amount", "")
            grant_amount = project_data.get("grant_amount", "")
            ...
            # Build identifiers (ADB project ID)
            identifiers = []
            if project_id:
                # Only add URL if it's valid and not empty
                url_value = project_data.get("url", "")
                if not url_value and project_id:
                    url_value = f"https://www.adb.org/projects/{project_id}"
                ...
                if url_value.strip():
                    identifiers.append(
                        ExternalIdentifier(
                            ...
                            name=LangText(
                                en=LangTextValue(
                                    value="ADB Project ID", provenance="human"
                                ),
                            ),
                        )
                    )
            ...
            if not slug_candidate or len(slug_candidate) < 3:
                if project_id:
                    slug_candidate = f"adb-{text_to_slug(str(project_id))}"
                else:
                    parts = [title_clean or title]
                    parts.append(district_name or province_name)
                    slug_candidate = text_to_slug("-".join([p for p in parts if p]))
                if not slug_candidate or len(slug_candidate) < 3:
                    slug_candidate = f"adb-{text_to_slug(str(project_id or 'unknown'))}"
            ...
            try:
                ...
                if "already exists" in msg:
                    ...
                    raise
            context.log(f"Created ADB project {project_entity.id}")
            created_entity_ids.append(project_entity.id)
            ...
            # Create FUNDED_BY relationship with Asian Development Bank
            try:
                # Create ADB organization entity for project funding relationship
                # Use a consistent slug to avoid duplicates
                adb_slug = "asian-development-bank"
                adb_entity = None

                # First try to create the entity, handle if it already exists
                try:
                    adb_entity = await context.publication.create_entity(
                        entity_type=EntityType.ORGANIZATION,
                        entity_subtype=EntitySubType.INTERNATIONAL_ORG,
                        entity_data={
                            "slug": adb_slug,
                            "names": [
                                Name(
                                    kind=NameKind.PRIMARY,
                                    en=NameParts(full="Asian Development Bank"),
                                    ne=NameParts(full="एशियाली विकास बैंक"),
                                ).model_dump()
                            ],
                            "attributions": attributions,
                            "description": LangText(
                                en=LangTextValue(
                                    value="Multilateral development bank focused on Asia and the Pacific",
                                    provenance="imported"
                                ),
                            ).model_dump(),
                        },
                        author_id=author_id,
                        change_description="Asian Development Bank organization entity",
                    )
                    context.log(f"Created ADB entity {adb_entity.id}")
                except ValueError as e:
                    if "already exists" in str(e):
                        # Entity already exists, we'll search for the existing entity
                        search_results = await context.search.search_entities(
                            entity_type="organization",
                            query=adb_slug,
                            limit=1
                        )
                        adb_entity = search_results[0] if search_results else None
                        context.log(f"ADB entity already exists, using existing entity {adb_entity.id if adb_entity else 'None'}")
                    else:
                        raise e  # Re-raise if it's a different error

                # Only create relationship if we have an ADB entity
                if adb_entity:
                    # Create AFFILIATED_WITH relationship (since FUNDED_BY is not a valid type)
                    rel = await context.publication.create_relationship(
                        source_entity_id=project_entity.id,
                        target_entity_id=adb_entity.id,
                        relationship_type="AFFILIATED_WITH",
                        author_id=author_id,
                        change_description=f"Funded by Asian Development Bank: {funding_source}",
                        attributes={
                            "funding_amount": total_budget,
                            "loan_amount": loan_amount,
                            "grant_amount": grant_amount,
                    ...
                    created_relationship_ids.append(rel.id)
                    context.log(
                        f"  Created AFFILIATED_WITH relationship: {project_entity.id} → {adb_entity.id}"
                    )
                else:
                    context.log("  Skipped AFFILIATED_WITH relationship: ADB entity not available")
            except Exception as e:
                context.log(
                    f"  ERROR: Failed to create FUNDED_BY relationship with ADB: {e}"
                )
                # Continue with other relationships even if one fails
                ...
                try:
                    ...
                    if not agency_entity:
                        ...
                        agency_entity = await context.publication.create_entity(
                            ...
                            author_id=author_id,
                            change_description="ADB project implementing agency",
                        )
                        context.log(f"Created implementing agency entity {agency_entity.id}")
                    ...
                    rel = await context.publication.create_relationship(
                        ...
                        relationship_type="AFFILIATED_WITH",
                        author_id=author_id,
                        change_description=f"Implements ADB project",
                        attributes={
                            "relationship_type": "IMPLEMENTS",  # Store original intent in attributes
                        }
                    )
            ...
            count += 1
            if count % 100 == 0:
                context.log(f"Processed {count} ADB projects...")

    except Exception as e:
        context.log(f"ERROR during ADB project migration: {e}")
        raise

    context.log(
        f"ADB migration completed: {count} projects created, {skipped_count} skipped, "
        f"{linked_count} locations linked, {relationships_count} relationships created"
    )


async def _migrate_jica_projects(context: MigrationContext, author_id: str) -> None:
    """
    Import JICA (Japan International Cooperation Agency) projects for Nepal from scraped JSON data

    Data source: JICA Yen Loan Database (yen_loan.csv converted to standardized JSON format)
    """
    context.log("Starting: Importing JICA projects for Nepal")

    # Load projects from pre-scraped data file
    context.log("Loading JICA projects from source data...")
    try:
        projects = context.read_json("source/jica_projects.json")
        context.log(
            f"Loaded {len(projects)} projects from source/jica_projects.json"
        )
    except FileNotFoundError:
        context.log("WARNING: source/jica_projects.json not found.")
        context.log(
            "Please run: python -m migrations.007-source-projects.jica.scrape_jica"
        )
        context.log("to scrape and save JICA project data first.")
        context.log("Skipping JICA project import...")
        return

    if not projects:
        context.log(
            "WARNING: No JICA projects in source data. Skipping import."
        )
        return
    ...
    import_date = datetime.now(timezone.utc).date()
    attribution_details = f"Imported from JICA (www.jica.go.jp) on {import_date}"
    attributions = [
        Attribution(
            title=LangText(
                en=LangTextValue(
                    value="JICA Projects", provenance="human"
                ),
                ne=LangTextValue(value="जेआईसीए परियोजना", provenance="human"),
            ),
            details=LangText(
                en=LangTextValue(value=attribution_details, provenance="human"),
                ne=LangTextValue(
                    value=f"जापान अन्तर्राष्ट्रिय सहयोग एजेन्सी (www.jica.go.jp) बाट {import_date} मा आयात गरिएको",
                    provenance="human",
                ),
            ),
        )
    ...
    # Get all locations for lookup (we can reuse the same lookup as World Bank, NPC, and ADB)
    locations = await context.search.search_entities(
        entity_type="location", limit=10_000
    )
    location_lookup: Dict[str, object] = {}
    ...
    try:
        for project_data in projects:
            # Extract basic information from JICA data format
            title = (project_data.get("title") or "").strip()
            if not title:
                skipped_count += 1
                continue
            ...
            # Extract funding information
            funding_source = project_data.get("funding_source", "Japan International Cooperation Agency")
            total_budget = project_data.get("total_allocated_budget", "")
            spending = project_data.get("real_time_spending", "")
            loan_amount = project_data.get("loan_amount", "")
            grant_amount = project_data.get("grant_amount", "")
            ...
            # Build identifiers (JICA project ID)
            identifiers = []
            if project_id:
                # Only add URL if it's valid and not empty
                url_value = project_data.get("url", "")
                if not url_value and project_id:
                    url_value = f"https://www.jica.go.jp/projects/{project_id}"
                ...
                if url_value.strip():
                    identifiers.append(
                        ExternalIdentifier(
                            ...
                            name=LangText(
                                en=LangTextValue(
                                    value="JICA Project ID", provenance="human"
                                ),
                            ),
                        )
                    )
            ...
            if not slug_candidate or len(slug_candidate) < 3:
                if project_id:
                    slug_candidate = f"jica-{text_to_slug(str(project_id))}"
                else:
                    parts = [title_clean or title]
                    parts.append(district_name or province_name)
                    slug_candidate = text_to_slug("-".join([p for p in parts if p]))
                if not slug_candidate or len(slug_candidate) < 3:
                    slug_candidate = f"jica-{text_to_slug(str(project_id or 'unknown'))}"
            ...
            try:
                ...
                if "already exists" in msg:
                    ...
                    raise
            context.log(f"Created JICA project {project_entity.id}")
            created_entity_ids.append(project_entity.id)
            ...
            # Create FUNDED_BY relationship with Japan International Cooperation Agency
            try:
                # Create JICA organization entity for project funding relationship
                # Use a consistent slug to avoid duplicates
                jica_slug = "jica-japan-international-cooperation-agency"
                jica_entity = None

                # First try to create the entity, handle if it already exists
                try:
                    jica_entity = await context.publication.create_entity(
                        entity_type=EntityType.ORGANIZATION,
                        entity_subtype=EntitySubType.INTERNATIONAL_ORG,
                        entity_data={
                            "slug": jica_slug,
                            "names": [
                                Name(
                                    kind=NameKind.PRIMARY,
                                    en=NameParts(full="Japan International Cooperation Agency"),
                                    ne=NameParts(full="जापान अन्तर्राष्ट्रिय सहयोग एजेन्सी"),
                                ).model_dump()
                            ],
                            "attributions": attributions,
                            "description": LangText(
                                en=LangTextValue(
                                    value="Japanese government agency that implements official development assistance (ODA) programs",
                                    provenance="imported"
                                ),
                            ).model_dump(),
                        },
                        author_id=author_id,
                        change_description="Japan International Cooperation Agency organization entity",
                    )
                    context.log(f"Created JICA entity {jica_entity.id}")
                except ValueError as e:
                    if "already exists" in str(e):
                        # Entity already exists, we'll search for the existing entity
                        search_results = await context.search.search_entities(
                        ...
                        jica_entity = search_results[0] if search_results else None
                        context.log(f"JICA entity already exists, using existing entity {jica_entity.id if jica_entity else 'None'}")
                    else:
                        raise e  # Re-raise if it's a different error

                # Only create relationship if we have a JICA entity
                if jica_entity:
                    # Create AFFILIATED_WITH relationship (since FUNDED_BY is not a valid type)
                    rel = await context.publication.create_relationship(
                        source_entity_id=project_entity.id,
                        target_entity_id=jica_entity.id,
                        relationship_type="AFFILIATED_WITH",
                        author_id=author_id,
                        change_description=f"Funded by Japan International Cooperation Agency: {funding_source}",
                        attributes={
                            "funding_amount": total_budget,
                            "loan_amount": loan_amount,
                            "grant_amount": grant_amount,
                    ...
                    created_relationship_ids.append(rel.id)
                    context.log(
                        f"  Created AFFILIATED_WITH relationship: {project_entity.id} → {jica_entity.id}"
                    )
                else:
                    context.log("  Skipped AFFILIATED_WITH relationship: JICA entity not available")
            except Exception as e:
                context.log(
                    f"  ERROR: Failed to create FUNDED_BY relationship with JICA: {e}"
                )
                # Continue with other relationships even if one fails
                ...
                try:
                    ...
                    if not agency_entity:
                        ...
                        agency_entity = await context.publication.create_entity(
                            ...
                            author_id=author_id,
                            change_description="JICA project implementing agency",
                        )
                        context.log(f"Created implementing agency entity {agency_entity.id}")
                    ...
                    rel = await context.publication.create_relationship(
                        ...
                        relationship_type="AFFILIATED_WITH",
                        author_id=author_id,
                        change_description=f"Implements JICA project",
                        attributes={
                            "relationship_type": "IMPLEMENTS",  # Store original intent in attributes
                        }
                    )
            ...
            count += 1
            if count % 100 == 0:
                context.log(f"Processed {count} JICA projects...")

    except Exception as e:
        context.log(f"ERROR during JICA project migration: {e}")
        raise

    context.log(
        f"JICA migration completed: {count} projects created, {skipped_count} skipped, "
        f"{linked_count} locations linked, {relationships_count} relationships created"
    )


async def _migrate_eu_projects(context: MigrationContext, author_id: str) -> None:
    """
    Import EU (European Union) projects for Nepal from scraped JSON data

    Data source: EU IATI XML feed and EU Aid Explorer API converted to standardized JSON format
    """
    context.log("Starting: Importing EU projects for Nepal")

    # Load projects from pre-scraped data file
    context.log("Loading EU projects from source data...")
    try:
        projects = context.read_json("source/eu_projects.json")
        context.log(
            f"Loaded {len(projects)} projects from source/eu_projects.json"
        )
    except FileNotFoundError:
        context.log("WARNING: source/eu_projects.json not found.")
        context.log(
            "Please run: python -m migrations.007-source-projects.european_union.scrape_eu"
        )
        context.log("to scrape and save EU project data first.")
        context.log("Skipping EU project import...")
        return

    if not projects:
        context.log(
            "WARNING: No EU projects in source data. Skipping import."
        )
        return
    ...
    import_date = datetime.now(timezone.utc).date()
    attribution_details = f"Imported from EU (European Union) on {import_date}"
    attributions = [
        Attribution(
            title=LangText(
                en=LangTextValue(
                    value="EU Projects", provenance="human"
                ),
                ne=LangTextValue(value="ईयू परियोजना", provenance="human"),
            ),
            details=LangText(
                en=LangTextValue(value=attribution_details, provenance="human"),
                ne=LangTextValue(
                    value=f"युरोपेली संघ (European Union) बाट {import_date} मा आयात गरिएको",
                    provenance="human",
                ),
            ),
        )
    ...
    # Get all locations for lookup (we can reuse the same lookup as other sources)
    locations = await context.search.search_entities(
        entity_type="location", limit=10_000
    )
    location_lookup: Dict[str, object] = {}
    ...
    try:
        for project_data in projects:
            # Extract basic information from EU data format
            title = (project_data.get("title") or "").strip()
            if not title:
                skipped_count += 1
                continue
            ...
            # Build identifiers (EU project ID)
            identifiers = []
            if project_id:
                # Only add URL if it's valid and not empty
                url_value = project_data.get("url", "")
                if not url_value and project_id:
                    url_value = f"https://data.europa.eu/eupedia/projects/{project_id}"
                ...
                if url_value.strip():
                    identifiers.append(
                        ExternalIdentifier(
                            ...
                            name=LangText(
                                en=LangTextValue(
                                    value="EU Project ID", provenance="human"
                                ),
                            ),
                        )
                    )
            ...
            if not slug_candidate or len(slug_candidate) < 3:
                if project_id:
                    slug_candidate = f"eu-{text_to_slug(str(project_id))}"
                else:
                    parts = [title_clean or title]
                    parts.append(district_name or province_name)
                    slug_candidate = text_to_slug("-".join([p for p in parts if p]))
                if not slug_candidate or len(slug_candidate) < 3:
                    slug_candidate = f"eu-{text_to_slug(str(project_id or 'unknown'))}"
            ...
            try:
                ...
                if "already exists" in msg:
                    ...
                    raise
            context.log(f"Created EU project {project_entity.id}")
            created_entity_ids.append(project_entity.id)
            ...
            # Create FUNDED_BY relationship with European Union
            try:
                # Create EU organization entity for project funding relationship
                # Use a consistent slug to avoid duplicates
                eu_slug = "european-union"
                eu_entity = None

                # First try to create the entity, handle if it already exists
                try:
                    eu_entity = await context.publication.create_entity(
                        entity_type=EntityType.ORGANIZATION,
                        entity_subtype=EntitySubType.INTERNATIONAL_ORG,
                        entity_data={
                            "slug": eu_slug,
                            "names": [
                                Name(
                                    kind=NameKind.PRIMARY,
                                    en=NameParts(full="European Union"),
                                    ne=NameParts(full="युरोपेली संघ"),
                                ).model_dump()
                            ],
                            "attributions": attributions,
                            "description": LangText(
                                en=LangTextValue(
                                    value="Political and economic union of 27 European countries that provides development aid",
                                    provenance="imported"
                                ),
                            ).model_dump(),
                        },
                        author_id=author_id,
                        change_description="European Union organization entity",
                    )
                    context.log(f"Created EU entity {eu_entity.id}")
                except ValueError as e:
                    if "already exists" in str(e):
                        # Entity already exists, we'll search for the existing entity
                        search_results = await context.search.search_entities(
                        ...
                        eu_entity = search_results[0] if search_results else None
                        context.log(f"EU entity already exists, using existing entity {eu_entity.id if eu_entity else 'None'}")
                    else:
                        raise e  # Re-raise if it's a different error

                # Only create relationship if we have an EU entity
                if eu_entity:
                    # Create AFFILIATED_WITH relationship (since FUNDED_BY is not a valid type)
                    rel = await context.publication.create_relationship(
                        source_entity_id=project_entity.id,
                        target_entity_id=eu_entity.id,
                        relationship_type="AFFILIATED_WITH",
                        author_id=author_id,
                        change_description=f"Funded by European Union: {funding_source}",
                        attributes={
                            "funding_amount": total_budget,
                            "loan_amount": loan_amount,
                            "grant_amount": grant_amount,
                    ...
                    created_relationship_ids.append(rel.id)
                    context.log(
                        f"  Created AFFILIATED_WITH relationship: {project_entity.id} → {eu_entity.id}"
                    )
                else:
                    context.log("  Skipped AFFILIATED_WITH relationship: EU entity not available")
            except Exception as e:
                context.log(
                    f"  ERROR: Failed to create FUNDED_BY relationship with EU: {e}"
                )
                # Continue with other relationships even if one fails
                ...
                try:
                    ...
                    if not agency_entity:
                        ...
                        agency_entity = await context.publication.create_entity(
                            ...
                            author_id=author_id,
                            change_description="EU project implementing agency",
                        )
                        context.log(f"Created implementing agency entity {agency_entity.id}")
                    ...
                    rel = await context.publication.create_relationship(
                        ...
                        relationship_type="AFFILIATED_WITH",
                        author_id=author_id,
                        change_description=f"Implements EU project",
                        attributes={
                            "relationship_type": "IMPLEMENTS",  # Store original intent in attributes
                        }
                    )
            ...
            count += 1
            if count % 100 == 0:
                context.log(f"Processed {count} EU projects...")

    except Exception as e:
        context.log(f"ERROR during EU project migration: {e}")
        raise

    context.log(
        f"EU migration completed: {count} projects created, {skipped_count} skipped, "
        f"{linked_count} locations linked, {relationships_count} relationships created"
    )


async def _migrate_dfmis_projects(context: MigrationContext, author_id: str) -> None:
    """
    Import MoF DFMIS (Ministry of Finance - Development Finance Information Management System) projects for Nepal from scraped JSON data

    Data source: MoF DFMIS API (dfims.mof.gov.np/api/v2/core/projects/) converted to standardized JSON format
    """
    context.log("Starting: Importing MoF DFMIS projects for Nepal")

    # Load projects from pre-scraped data file
    context.log("Loading MoF DFMIS projects from source data...")
    try:
        projects = context.read_json("source/dfmis_projects.json")
        context.log(
            f"Loaded {len(projects)} projects from source/dfmis_projects.json"
        )
    except FileNotFoundError:
        context.log("WARNING: source/dfmis_projects.json not found.")
        context.log(
            "Please run: python -m migrations.007-source-projects.mof_dfmis.scrape_mof_dfmis"
        )
        context.log("to scrape and save MoF DFMIS project data first.")
        context.log("Skipping MoF DFMIS project import...")
        return

    if not projects:
        context.log(
            "WARNING: No MoF DFMIS projects in source data. Skipping import."
        )
        return
    ...
    import_date = datetime.now(timezone.utc).date()
    attribution_details = f"Imported from MoF DFMIS (dfims.mof.gov.np) on {import_date}"
    attributions = [
        Attribution(
            title=LangText(
                en=LangTextValue(
                    value="MoF DFMIS Projects", provenance="human"
                ),
                ne=LangTextValue(value="MoF DFMIS परियोजना", provenance="human"),
            ),
            details=LangText(
                en=LangTextValue(value=attribution_details, provenance="human"),
                ne=LangTextValue(
                    value=f"मालाे (dfims.mof.gov.np) बाट {import_date} मा आयात गरिएको",
                    provenance="human",
                ),
            ),
        )
    ...
    try:
        for project_data in projects:
            # Extract basic information from DFMIS data format
            title = (project_data.get("title") or "").strip()
            if not title:
                skipped_count += 1
                continue
            ...
            # Build identifiers (DFMIS project ID)
            identifiers = []
            if project_id:
                # Only add URL if it's valid and not empty
                url_value = project_data.get("url", "")
                if not url_value and project_id:
                    url_value = f"https://dfims.mof.gov.np/projects/{project_id}"
            ...
            if not slug_candidate or len(slug_candidate) < 3:
                if project_id:
                    slug_candidate = f"dfmis-{text_to_slug(str(project_id))}"
                else:
                    parts = [title_clean or title]
                    parts.append(district_name or province_name)
                    slug_candidate = text_to_slug("-".join([p for p in parts if p]))
                if not slug_candidate or len(slug_candidate) < 3:
                    slug_candidate = f"dfmis-{text_to_slug(str(project_id or 'unknown'))}"
            ...
            try:
                ...
                if "already exists" in msg:
                    ...
                    raise
            context.log(f"Created MoF DFMIS project {project_entity.id}")
            created_entity_ids.append(project_entity.id)
            ...
            # Create FUNDED_BY relationship with funding organization
            try:
                # Determine appropriate funding organization based on funding source
                funding_source_clean = funding_source.split(",")[0].strip() if funding_source else "Government of Nepal"
                funding_slug = text_to_slug(funding_source_clean)
                funding_entity = None

                # First try to create the entity, handle if it already exists
                try:
                    funding_entity = await context.publication.create_entity(
                        entity_type=EntityType.ORGANIZATION,
                        entity_subtype=EntitySubType.GOVERNMENT_AGENCY if "Government of Nepal" in funding_source or "Ministry" in funding_source else EntitySubType.INTERNATIONAL_ORG,
                        entity_data={
                            "slug": funding_slug,
                            "names": [
                                Name(
                                    kind=NameKind.PRIMARY,
                                    en=NameParts(full=funding_source_clean),
                                    ne=NameParts(full=funding_source_clean),  # Would need translation
                                ).model_dump()
                            ],
                            "attributions": attributions,
                            "description": LangText(
                                en=LangTextValue(
                                    value=f"Funding organization for development projects in Nepal",
                                    provenance="imported"
                                ),
                            ).model_dump(),
                        },
                        author_id=author_id,
                        change_description="MoF DFMIS funding organization entity",
                    )
                    context.log(f"Created funding entity {funding_entity.id}")
                except ValueError as e:
                    if "already exists" in str(e):
                        # Entity already exists, we'll search for the existing entity
                        search_results = await context.search.search_entities(
                        ...
                        funding_entity = search_results[0] if search_results else None
                        context.log(f"Funding entity already exists, using existing entity {funding_entity.id if funding_entity else 'None'}")
                    else:
                        raise e  # Re-raise if it's a different error

                # Only create relationship if we have a funding entity
                if funding_entity:
                    # Create AFFILIATED_WITH relationship (since FUNDED_BY is not a valid type)
                    rel = await context.publication.create_relationship(
                        source_entity_id=project_entity.id,
                        target_entity_id=funding_entity.id,
                        relationship_type="AFFILIATED_WITH",
                        author_id=author_id,
                        change_description=f"Funded by {funding_source_clean}",
                        attributes={
                            "funding_amount": total_budget,
                            "loan_amount": loan_amount,
                            "grant_amount": grant_amount,
                    ...
                    created_relationship_ids.append(rel.id)
                    context.log(
                        f"  Created AFFILIATED_WITH relationship: {project_entity.id} → {funding_entity.id}"
                    )
                else:
                    context.log("  Skipped AFFILIATED_WITH relationship: Funding entity not available")
            except Exception as e:
                context.log(
                    f"  ERROR: Failed to create FUNDED_BY relationship with funding source: {e}"
                )
                # Continue with other relationships even if one fails
                ...
                try:
                    ...
                    if not agency_entity:
                        ...
                        agency_entity = await context.publication.create_entity(
                            ...
                            author_id=author_id,
                            change_description="MoF DFMIS project implementing agency",
                        )
                        context.log(f"Created implementing agency entity {agency_entity.id}")
                    ...
                    rel = await context.publication.create_relationship(
                        ...
                        relationship_type="AFFILIATED_WITH",
                        author_id=author_id,
                        change_description=f"Implements MoF DFMIS project",
                        attributes={
                            "relationship_type": "IMPLEMENTS",  # Store original intent in attributes
                        }
                    )
            ...
            count += 1
            if count % 100 == 0:
                context.log(f"Processed {count} MoF DFMIS projects...")

    except Exception as e:
        context.log(f"ERROR during MoF DFMIS project migration: {e}")
        raise

    context.log(
        f"MoF DFMIS migration completed: {count} projects created, {skipped_count} skipped, "
        f"{linked_count} locations linked, {relationships_count} relationships created"
    )