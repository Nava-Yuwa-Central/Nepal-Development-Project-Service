...
class WorldBankProjectScraper:
    ...
    FINANCESONE_API_URL = "https://datacatalogapi.worldbank.org/dexapps/fone/summary/ibrd/lending/table"
    ...
    async def search_nepal_projects(self, fiscal_year: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        async with self.client:
            ...
            logger.info(f"Successfully scraped {len(combined_projects)} projects for Nepal")
            return combined_projects
    
    async def _fetch_projects_from_main_api(self, fiscal_year: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if isinstance(data, int):
                logger.warning(f"Received integer response from API (likely status code): {data}")
                return []
            if isinstance(data, dict):
                ...
                if "projects" in data and isinstance(data["projects"], dict):
                    ...
                    for project in projects_list:
                        ...
                        is_nepal = (
                            (self.nepal_country_code in country_code if isinstance(country_code, list) else self.nepal_country_code == country_code) or
                            self.nepal_country_code in str(country_code) or
                            "Nepal" in country_name or
                            "Nepal" in country_short
                        )

                        # Additional check: exclude if it's clearly another country (like Liberia, India, etc.)
                        is_other_country = (
                            "Liberia" in country_name or "Liberia" in country_short or
                            "India" in country_name or "India" in country_short or
                            "Sri Lanka" in country_name or "Sri Lanka" in country_short or
                            "Bangladesh" in country_name or "Bangladesh" in country_short or
                            "Pakistan" in country_name or "Pakistan" in country_short or
                            "Myanmar" in country_name or "Myanmar" in country_short or
                            "Bhutan" in country_name or "Bhutan" in country_short or
                            "Afghanistan" in country_name or "Afghanistan" in country_short or
                            "Nigeria" in country_name or "Nigeria" in country_short or
                            "Ghana" in country_name or "Ghana" in country_short
                        )
                        ...
                        if is_nepal and not is_other_country:
                            ...
                            logger.debug(f"Excluding project {project.get('id', 'unknown')} - appears to be for another country despite Nepal filter")

                    logger.info(f"Filtered to {len(nepal_projects)} projects actually for Nepal out of {len(projects_list)} total projects")
                ...
                elif "total_rows" in data and "rows" in data:  # Common API response format
                    return data["rows"]
                else:
                ...
                logger.warning(f"Received unexpected response type from API: {type(data)}")
                return []
        except Exception as e:
        ...
        try:
            ...
            async with self.client.session.post(
                ...
                if response.status == 200:
                    ...
                    logger.warning(f"FinancesOne API request failed with status {response.status}")
                    return []
        except Exception as e:
    ...
    def _process_financesone_data(self, data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for item in lending_items:
            if isinstance(item, dict):
                ...
                if (country_code == self.nepal_country_code or
                    "Nepal" in country_name):
                    processed_item = {
                        "project_id": item.get("ProjectId", item.get("project_id", item.get("id", ""))),
                        "project_name": item.get("ProjectName", item.get("project_name", item.get("name", ""))),
                        "lending_instrument": item.get("Product", item.get("lending_instrument", "")),
                        "loan_amount": item.get("Principal", item.get("loan_amount", item.get("amount", ""))),
                        "grant_amount": item.get("grant_amount", ""),
                        "status": item.get("Status", item.get("status", "")),
                        "fiscal_year": item.get("FiscalYear", item.get("fiscal_year", item.get("year", ""))),
                        "sector": item.get("sector", ""),
                        "implementing_agency": item.get("implementing_agency", ""),
                        "borrower": item.get("Borrower", item.get("borrower", "")),
                        "closing_date": item.get("ClosedDate", item.get("closing_date", "")),
                        "disbursement": item.get("Disbursed", item.get("disbursement", "")),
                        "repayment_commencement_date": item.get("repayment_commencement_date", ""),
    ...
    def normalize_project_data(self, raw_project: Dict[str, Any]) -> Dict[str, Any]:
        """Normalize raw World Bank project data to the standard format.

        Args:
            raw_project: Raw project data from World Bank API

        Returns:
            Normalized project data matching the PTP requirements
        """
        # Extract nested fields from complex structures in the search API response
        sector_info = raw_project.get("major_sectors", [])
        sector_names = []
        if sector_info and isinstance(sector_info, list) and len(sector_info) > 0:
            major_sector = sector_info[0].get("major_sector", {})
            sectors = major_sector.get("sectors", [])
            if sectors:
                sector_names = [s.get("sector_name", "") for s in sectors if s.get("sector_name")]

        # Extract theme information
        themes = raw_project.get("themev2_level1_exact", [])
        theme1_name = themes[0] if themes else raw_project.get("major_theme", "")

        # Extract country information - ensure it's for Nepal
        country_code = raw_project.get("countrycode", [])
        if isinstance(country_code, list):
            actual_country_code = country_code[0] if country_code else self.nepal_country_code
        else:
            actual_country_code = str(country_code) if country_code else self.nepal_country_code

        normalized = {
            # Core project information (from PTP requirements)
            "project_id": raw_project.get("project_id",
                                        raw_project.get("proj_id",
                                                      raw_project.get("id",
                                                                    raw_project.get("projectnumber", "")))),
            "title": raw_project.get("project_name",
                                   raw_project.get("projectname",
                                                 raw_project.get("project_name",
                                                               raw_project.get("project_name", "")))),
            "description": raw_project.get("project_abstract",
                                         raw_project.get("description",
                                                       raw_project.get("projectstatus", ""))),

            # Implementing agency
            "implementing_agency": raw_project.get("implementing_agency",
                                                  raw_project.get("impagency",
                                                                raw_project.get("curr_impagency", ""))),

            # Timeline & milestones (from PTP requirements)
            "start_date": self._parse_date(raw_project.get("board_approval_date",
                                                         raw_project.get("boardapprovaldate",
                                                                       raw_project.get("approval_date",
                                                                                     raw_project.get("start_date", ""))))),
            "end_date": self._parse_date(raw_project.get("project_completion_date",
                                                        raw_project.get("closingdate",
                                                                      raw_project.get("board_decision_date", "")))),
            "milestones": self._extract_milestones(raw_project),

            # Location information (from PTP requirements)
            "location": {
                "country": raw_project.get("countryname", raw_project.get("countryshortname", "Nepal")),
                "country_code": actual_country_code if actual_country_code == self.nepal_country_code else self.nepal_country_code,
                "region": raw_project.get("regionname", raw_project.get("region_name", "")),
                "province": "",  # Will need to be extracted from project data if available
                "district": "",  # Will need to be extracted from project data if available
                "municipality": ""  # Will need to be extracted from project data if available
            },

            # Financial transparency (from PTP requirements)
            "funding_source": "World Bank",  # Will be expanded when combined with other data sources
            "total_allocated_budget": self._extract_budget_amount(raw_project),
            "yearly_budget_breakdown": self._extract_yearly_budget(raw_project),
            "real_time_spending": raw_project.get("total_disbursed",
                                                raw_project.get("totalcommamt",
                                                              raw_project.get("curr_total_commitment", ""))),
            "cost_overruns": self._calculate_cost_overruns(raw_project),

            # Progress monitoring (from PTP requirements)
            "physical_progress": raw_project.get("progress_status", ""),
            "financial_progress": self._calculate_financial_progress(raw_project),
            "photos": raw_project.get("project_photos", []),
            "reports": self._extract_reports(raw_project),
            "verification_documents": raw_project.get("docs_url",
                                                    raw_project.get("countryhomepageurl", "")),
            "contractor_change_log": [],  # Not typically available in World Bank data

            # Additional project metadata
            "borrower": raw_project.get("borrower", ""),
            "environmental_category": raw_project.get("envassesmentcategorycode",
                                                    raw_project.get("esrc_ovrl_risk_rate",
                                                                  raw_project.get("env_assess_proc", ""))),
            "major_theme": theme1_name,
            "major_theme_sector": theme1_name,
            "sector": raw_project.get("sector_name",
                                    raw_project.get("mjsector",
                                                  ", ".join(sector_names) if sector_names else
                                                  raw_project.get("sector1", ""))),

            # URLs and documents
            "url": raw_project.get("url",
                                 raw_project.get("project_url",
                                               f"https://projects.worldbank.org/en/projects/{raw_project.get('proj_id', raw_project.get('id', ''))}")),
            "project_document_url": raw_project.get("project_doc_url",
                                                   raw_project.get("docs_url",
                                                                 raw_project.get("regionhomepageurl", ""))),
            "implementation_status": raw_project.get("status",
                                                    raw_project.get("projectstatus",
                                                                  raw_project.get("impstatus",
                                                                                raw_project.get("status_exact", [""])[0] if isinstance(raw_project.get("status_exact"), list) else ""))),
            ...
            # Source tracking
            "source": "World Bank",
            "source_api": "Projects & Operations API or FinancesOne API",
        }
    ...
    def save_to_migration_file(self, data: List[Dict[str, Any]]):
        """Save scraped data to the file expected by the migration.
        ...
        logger.info(f"Saved {len(data)} projects to migration file: {migration_file_path}")
    ...
    def _extract_milestones(self, raw_project: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        key_dates = [
            ...
            ("Start Date", raw_project.get("actual_start_date", raw_project.get("start_date"))),
            ("Closing Date", raw_project.get("project_completion_date", raw_project.get("closingdate"))),
        ...
        for name, date in key_dates:
            if date:
                milestones.append({
                    ...
                    "status": "Completed" if datetime.now() >= datetime.strptime(self._parse_date(date)[:10], "%Y-%m-%d") else "Planned",
                    "description": f"{name} of the project"
                })

        return milestones

    def _extract_yearly_budget(self, raw_project: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def _calculate_cost_overruns(self, raw_project: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        total_allocated = raw_project.get("total_amount", raw_project.get("totalamt", ""))
        total_disbursed = raw_project.get("total_disbursed", raw_project.get("totalcommamt", ""))
    ...
    def _extract_budget_amount(self, raw_project: Dict[str, Any]) -> str:
        ...
        amount_fields = [
            "total_amount", "totalamt", "amount", "lending_amount",
            "loan_amount", "ibrdamt", "ida_amount", "grant_amount", "grantamt"
        ]

        for field in amount_fields:
            if field in raw_project and raw_project[field]:
                return str(raw_project[field])
